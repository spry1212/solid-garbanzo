 ...
 */
// ── NEW ADVANCED STATES (Add near other state definitions) ─────────────────
const [currentWeekOffset, setCurrentWeekOffset] = useState(0); // -1 = past, 0 = current, 1 = future
const [shopSortMode, setShopSortMode] = useState("category"); // 'category' or 'vendor'

// Stores structure: { [ingredientKey]: { brand: "Coles Organic", vendor: "Coles" } }
const [ingredientPreferences, setIngredientPreferences] = useState(() => 
  recall("ingredient_prefs", {})
);

// Toggle state to manage the custom eating out modal / input row
const [editingOutSlot, setEditingOutSlot] = useState(null); // { day, slot }
const [outNoteText, setOutNoteText] = useState("");

// Update your main plan storage calculation to read from a dynamically keyed week offset
const [weeklyPlans, setWeeklyPlans] = useState(() => {
  return recall("weekly_plans_v1", {
    "week_0": recall("plan", {}) // Backwards compatibility with your existing plan
  });
});

// Sync changes to localStorage
useEffect(() => {
  persist("ingredient_prefs", ingredientPreferences);
}, [ingredientPreferences]);

useEffect(() => {
  persist("weekly_plans_v1", weeklyPlans);
}, [weeklyPlans]);

// Extract active plan based on offset
const currentWeekKey = `week_${currentWeekOffset}`;
const activePlan = weeklyPlans[currentWeekKey] || {};

// Function to update single slots inside specific weeks safely
const updateActivePlanSlot = (day, slot, value) => {
  setWeeklyPlans(prev => ({
    ...prev,
    [currentWeekKey]: {
      ...prev[currentWeekKey],
      [day]: {
        ...(prev[currentWeekKey]?.[day] || {}),
        [slot]: value
      }
    }
  }));
...
// Enhanced internal shopping list processing
const processedShopList = Object.entries(buildShopList(activePlan, servings)).map(([key, item]) => {
  const prefs = ingredientPreferences[key] || { brand: "Generic/Standard", vendor: "Any Supermarket" };
  return {
    key,
    ...item,
    brand: prefs.brand,
...
const handleExportList = (target) => {
  let outputText = `📅 MayaPlan Shopping List - ${currentWeekOffset === 0 ? "This Week" : currentWeekOffset > 0 ? "Future Week +" + currentWeekOffset : "Past Week " + currentWeekOffset}\n\n`;
  
  if (target === "keep") {
    processedShopList.forEach(i => {
      if (!i.checked) outputText += `[ ] ${i.qty}${i.unit} ${i.name} (${i.brand} @ ${i.vendor})\n`;
    });
  } else {
    // Filter specifically down to matching vendors or aggregate cleanly for grocery apps
    const filteredItems = processedShopList.filter(i => 
      target === "any" || i.vendor.toLowerCase().includes(target.toLowerCase())
    );
    
    outputText += `🛒 Target Store: ${target.toUpperCase()}\n`;
    filteredItems.forEach(i => {
      if (!i.checked) outputText += `• ${i.name} - ${i.qty}${i.unit} (${i.brand})\n`;
    });
  }
  
  navigator.clipboard.writeText(outputText);
  alert(`📋 List exported successfully to your Clipboard for ${target}! Paste directly into your notes or store manifest app.`);
};
}
...
export default function App() {
  ...
  function togglePin(day, slot) {
    ...
    }// Add inside your daily slots rendering loop
if (meal?.eatingOut) {
  return (
    <div key={slot} style={{ ...cardStyle, background: surfaceBg, padding: 12, minHeight: 80, display: "flex", flexDirection: "column", justifyContent: "center", borderLeft: "4px solid " + goldM }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <span style={{ fontSize: 10, fontWeight: 700, textTransform: "uppercase", color: gold }}>🍽 {slot} - Eating Out</span>
        <button 
          onClick={() => updateActivePlanSlot(day, slot, null)} 
          style={{ background: "none", border: "none", color: terra, cursor: "pointer", fontSize: 11 }}
        >
          Reset Slot
        </button>
      </div>
      <div style={{ fontSize: 12, fontStyle: "italic", color: mid, marginTop: 4 }}>
        "{meal.note || "Special Occasion / Blanked Out"}"
      </div>
    </div>
  )
  }
  ...
  // ── PLAN TAB ──────────────────────────────────────────────────────────────
  {/* Multi-Week Timeline Pager Element */}
<div style={{ ...cardStyle, padding: 12, display: "flex", justifyContent: "space-between", alignItems: "center", background: cardBg, marginBottom: 16 }}>
  <button onClick={() => setCurrentWeekOffset(p => p - 1)} style={btn(surfaceBg, charcoal)}>◀ Previous Week</button>
  <div style={{ textAlign: "center" }}>
    <span style={{ ...tag(sage, sageL), marginBottom: 4 }}>TIMELINE HORIZON</span>
    <div style={{ fontWeight: 700, fontSize: 14, color: charcoal }}>
      {currentWeekOffset === 0 ? "📅 Active Current Week" : currentWeekOffset === 1 ? "🚀 Next Week's Preparation" : `Horizon Offset Loop (${currentWeekOffset})`}
    </div>
  </div>
  <button onClick={() => setCurrentWeekOffset(p => p + 1)} style={btn(surfaceBg, charcoal)}>Next Week ▶</button>
</div>
function PlanTab() {
...
<button 
  onClick={() => {
    const note = prompt("Enter a note for this special occasion (e.g., Business Lunch at Cremorne, Dinner Party):");
    if (note !== null) {
      updateActivePlanSlot(day, slot, { eatingOut: true, note: note || "Eating Out" });
    }
  }} 
  style={btn("transparent", mid, true)}
>
  🥂 Flag Special Occasion
</button>
  ...
  const [tempPref, setTempPref] = useState({ brand: "", vendor: "" });

  // Categorize or cluster items according to active sort rules
  const groupedItems = {};
  processedShopList.forEach(item => {
    const groupKey = shopSortMode === "category" ? item.cat : item.vendor;
  ...
  const savePref = (key) => {
    setIngredientPreferences(prev => ({
      ...prev,
      [key]: { brand: tempPref.brand || "Generic", vendor: tempPref.vendor || "Any Supermarket" }
    }));
    setEditingKey(null);
  };
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      {/* Action Header & Integrations */}
      <div style={{ ...cardStyle, padding: 16, background: surfaceBg }}>
        <div style={{ display: "flex", flexWrap: "wrap", justifyContent: "space-between", alignItems: "center", gap: 12 }}>
          <div>
            <h2 style={{ margin: "0 0 4px", fontSize: 16, color: sage }}>Shopping Round Orchestrator</h2>
            <p style={{ margin: 0, fontSize: 12, color: mid }}>Organise items by aisle or vendor pathways and manage your third-party provisions.</p>
          </div>
          
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            <button onClick={() => setShopSortMode("category")} style={btn(shopSortMode === "category" ? sage : rule, shopSortMode === "category" ? "#fff" : charcoal)}>📁 Group by Aisle</button>
            <button onClick={() => setShopSortMode("vendor")} style={btn(shopSortMode === "vendor" ? sage : rule, shopSortMode === "vendor" ? "#fff" : charcoal)}>🏪 Group by Vendor</button>
          </div>
        </div>

        <div style={{ marginTop: 14, paddingTop: 14, borderTop: "1px dashed " + rule, display: "flex", gap: 8, flexWrap: "wrap" }}>
          <span style={{ fontSize: 11, fontWeight: "bold", color: mid, alignSelf: "center" }}>EXPORT EXTERNAL:</span>
          <button onClick={() => handleExportList("coles")} style={btn(blue, "#fff", true)}>🧡 Export to Coles</button>
          <button onClick={() => handleExportList("woolworths")} style={btn(teal, "#fff", true)}>💚 Export to Woolworths</button>
          <button onClick={() => handleExportList("keep")} style={btn(gold, "#fff", true)}>📝 Format for Google Keep</button>
        </div>
      </div>

      {/* Structured List Output */}
      {Object.keys(groupedItems).length === 0 ? (
        <div style={{ textAlign: "center", padding: "40px 20px", ...cardStyle, color: light }}>
          No planned ingredients found for this week. Generate or select a meal plan block to begin.
        </div>
      ) : (
        Object.entries(groupedItems).map(([groupTitle, items]) => (
          <div key={groupTitle} style={cardStyle}>
            <div style={{ background: sageXL, padding: "8px 16px", borderBottom: "1px solid " + rule, fontWeight: 700, textTransform: "uppercase", fontSize: 11, color: sage, display: "flex", justifyContent: "space-between" }}>
            ...
            <div style={{ padding: "4px 0" }}>
              {items.map(item => (
                <div key={item.key} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 16px", borderBottom: "1px solid #f0efe9" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10, flex: 1 }}>
                    <input 
                      type="checkbox" 
                      checked={item.checked} 
                      onChange={() => {
                        // Locate target meal component and toggle dynamic item configuration state if built into your system
                      }}
                      style={{ cursor: "pointer" }}
                    />
                    <div>
                      <span style={{ fontWeight: 600, fontSize: 13, textDecoration: item.checked ? "line-through" : "none", color: item.checked ? light : charcoal }}>
                        {item.qty ? `${Math.round(item.qty * 100) / 100} ${item.unit}` : ""} {item.name}
                      </span>
                      <div style={{ fontSize: 11, color: mid, marginTop: 2 }}>
                        Pref: <span style={{ color: terra, fontWeight: 500 }}>{item.brand}</span> @ <span style={{ color: sage, fontWeight: 500 }}>{item.vendor}</span>
                      </div>
                    </div>
                  </div>

                  {/* Inline preferences setting modal swap */}
                  {editingKey === item.key ? (
                    <div style={{ display: "flex", gap: 4, alignItems: "center" }}>
                      <input 
                        style={{ ...selStyle, width: 90 }} 
                        placeholder="Brand" 
                        value={tempPref.brand} 
                        onChange={e => setTempPref(p => ({ ...p, brand: e.target.value }))}
                      />
                      <select 
                        style={selStyle} 
                        value={tempPref.vendor} 
                        onChange={e => setTempPref(p => ({ ...p, vendor: e.target.value }))}
                      >
                        <option value="Coles">Coles</option>
                        <option value="Woolworths">Woolworths</option>
                        <option value="Local Deli/Butcher">Deli/Butcher</option>
                        <option value="South Melbourne Market">Sth Melb Market</option>
                      </select>
                      <button onClick={() => savePref(item.key)} style={btn(sage, "#fff", true)}>✔</button>
                    </div>
                  ) : (
                    <button 
                      onClick={() => {
                        setEditingKey(item.key);
                        setTempPref({ brand: item.brand, vendor: item.vendor });
                      }} 
                      style={btn(surfaceBg, mid, true)}
                    >
                      ⚙ Prefs
                    </button>
                  )}
